...
my $version = "tex2png.pl v1.6.4 Charles Plager".
...
## Revision History 
##
## 1.6.4 070612 cplager
## - added '-input' and '-noresolution' options
##
## 1.6.3 070307 cplager
## - added '-nohtml' option
##
## 1.6.2  060526 cplager
## - added '-ps' option
##
...
## print out HTML code by default
my $noHtml = "";
...
my ($mathmode, $useTransparent, $useStdin) = 
  ("true", "white", "true");
my ($output, $debug, $color, $extract, $extractPng, $keepPs, $noResolution);
my $argline = "@ARGV";
my (@packages, @inputs);
while (@ARGV && $ARGV[0] =~ /^-/) {
   $_ = shift(@ARGV);
   ...
   print "I do not understand option '$_'.  Aborting.\n\n";
   help();
} # while -arg

my $package = "";
if (@packages) {
   $package = '\usepackage{'. join(',', @packages)."}\n";
}
...
usage: $thiscommand [ options ] [texfile]
  Options:
      ...
      -input XXX        Includes file 'xxx.tex' (can be used 
                        multiple times)
      -nohtml           Does not print out HTML code
      -noresolution     Prints out <img src> tag without specifying resolution
      -output bla       Outputs 'bla.png'
      -package XXX      Includes package 'XXX' in header (can be used 
                        multiple times)
      -png              Generate PNG image (default)
      -ps               Leave PS file as well as PNG/GIF
